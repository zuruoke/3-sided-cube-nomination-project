...
const Modal: React.FC<ModalProps> = ({ isOpen, onConfirm, onCancel }) => {
  ...
  return (
    <div className='fixed inset-0 flex items-center justify-center z-50'>
      ...
      {/* Background overlay */}
      <div className='bg-white p-8 rounded-lg shadow-lg relative z-10 w-96'>
        {' '}
        {/* Modal content */}
        <h2 className={`text-2xl font-bold mb-4 ${poppins.className}`}>
          ARE YOU SURE?
        </h2>
        <p className={`mb-6 ${anonymous.className}`}>
          If you leave this page, you will lose any progress made.
        </p>
        <div className='flex flex-col space-y-4'>
          <StickyButtons onClick={onConfirm} text='YES, LEAVE PAGE' />
